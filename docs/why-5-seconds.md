...
### The Root Cause: Scan Once, Query on Index

The single reason dexfinder is fast:

**Separate scanning from querying. One full scan builds three in-memory maps. Every subsequent operation — query matching, call chain tracing, hidden API detection, reflection detection — is just index lookups on these maps. The raw DEX bytecode is never touched again.**
...
```
            APK (248MB, 16 DEX)
                    ...
                    ▼
        ┌───────────────────────┐
        │   Full Scan (3.7s)    │   one pass through every instruction
        └───────────┬───────────┘
                    │
        ┌───────────┼───────────┐
        ▼           ▼           ▼
   MethodRefs   FieldRefs   StringRefs       ← the three maps = the index
   431K          551K        414K
        │           │           │
        └───────────┼───────────┘
                    │
                    ▼
            Any query: O(N) map scan
            Call graph: reverse index on MethodRefs
            Hidden API: filter MethodRefs against CSV
            Reflection: reverse index CSV class→members against StringRefs
            All < 1 second
```

veridex has no such separation. Its scanning, type resolution, and matching are a coupled pipeline — every API check goes back to the DEX data.

Everything below is a consequence of this design:
...
**Separate scanning from querying. Build the index once, run N queries on it.**

veridex couples scanning and matching — every run walks from bytecode to results. We convert DEX bytecode into three generic maps, and all subsequent operations work on the maps, never touching the raw data again.

Same idea as a database: once the index is built, `SELECT` speed depends on the result set, not the table size.
...
**一次全量扫描建三个内存 map，后续所有操作——查询匹配、调用链追踪、Hidden API 检测、反射检测——都在 map 上做索引查找，不再碰原始字节码。**
...
```
            APK (248MB, 16 DEX)
                    ...
                    ▼
        ┌───────────────────────┐
        │   全量扫描 (3.7s)      │  遍历每条指令，只做一次
        └───────────┬───────────┘
                    │
        ┌───────────┼───────────┐
        ▼           ▼           ▼
   MethodRefs   FieldRefs   StringRefs       ← 三个 map = 索引
   43万条        55万条       41万条
        │           │           │
        └───────────┼───────────┘
                    │
                    ▼
            查询: 遍历 map key 做字符串匹配
            调用图: MethodRefs 的反向索引
            Hidden API: MethodRefs 过滤 CSV
            反射检测: CSV class→members 反向索引查 StringRefs
            以上全部 < 1 秒
```

查 `getDeviceId` 是 3.7 + 0.01 秒。查 `LauncherUI.onCreate` 也是 3.7 + 0.01 秒。加上 Hidden API 反射检测是 3.7 + 0.3 秒。**查什么都一样快，因为扫描和查询彻底解耦。**

veridex 没有这层分离。它的扫描、类型解析、匹配是一个耦合的流水线——每次检查都要回到 DEX 数据上重新 resolve。
...
**扫描和查询分离。一次建索引，N 次查询都是索引操作。**

veridex 把扫描和匹配耦合在一起——每次运行都从字节码走到结果。我们把 DEX 字节码转成三个通用 map，后续所有操作都在 map 上完成，不再碰原始数据。

这和数据库的思路一样：建好索引之后，`SELECT` 的速度和表的大小无关，只和结果集大小有关。
